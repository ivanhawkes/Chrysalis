...
REINST("Need to get this struct working with the vec3 defines, it causes link errors presently...ILH");
...
ILINE Vec3 _FindVectorToClosestPointOnLedge(const Vec3 &referencePoint, const SLedgeInfo &ledgeInfo)
{
	...
	// This is normalizing so projection below is valid.
	const float halfWidth = (vXDir.NormalizeSafe() * 0.5f);

	// We use this to ensure when adjusting ends of a ledge. A ledge will always be at least half its size
	// if the desired adjust amount is greater than the ledge size.
...
ILINE bool IsBestLedge(const Vec3& positionToLedge, const Vec3& testDirection, const SLedgeInfo& ledgeInfo,
	const float bestDistanceSq, const float cosMaxAngle, const bool enabled, float& newBestDistanceSq)
{
	...
	// Note: We do the enable check at the end, because, 99% of the time we don't reach this point, we save
	// the extra branching while looping through those ledges.
...
void DrawLedge(IRenderAuxGeom* pRenderAuxGeometry, const Vec3 startPoint, const Vec3& endPoint, const Vec3& facingDirection,
	const ELedgeFlagBitfield flags [LedgeSide_Count])
{
	...
	for (uint32 objectIdx = 0; objectIdx < ledgeObjectCount; ++objectIdx)
	{
		...
		do
		{
			for (uint32 markerIdx = 0; markerIdx < (markersCount - 1); ++markerIdx)
			{
				...
				// The item can be skipped if the angle is too big. Since only the cosine of angles are compared,
...
void CLedgeManager::Load(const char* fileName)
{
	...
	if (false != file.Open(fileName, "rb"))
	{
		...
		{
			// TODO: need way to display game warnings put back into the system.
		...
		// TODO: ILH - I took out an array indirection here [0], because it shouldn't be needed and was causing
		// a linker missing external error. Check to ensure nothing was broken by doing this.
		/*file.ReadType (&m_levelLedges.m_pLedgeObjects, totalLedgeObjectsCount);
		file.ReadType (&m_levelLedges.m_pMarkers, totalLedgeMarkersCount);*/
...
void CLedgeManager::UpdateLedgeMarkers(const EntityId entityId, const SLedgeMarker* pMarkersArray, const uint32 markerCount)
{
	...
	else
	{
		...
		if ((pLedgeObject != nullptr) && (pLedgeObject->m_markersCount == markerCount))
		{
...
LedgeId CLedgeManager::FindNearestLedge(const Vec3 &referencePosition, const Vec3 &testDirection, float maxDistance,
	float angleRange, float extendedAngleRange) const
{
	if (m_editorManager.IsInEditorMode())
	{
