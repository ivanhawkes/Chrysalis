...
void CWaterRipplesGenerator::Update(SEntityUpdateContext &ctx, int updateSlot)
{
	...
	float fFrequency = m_frequency + (cry_random(0.0f, 1.0f) * 2 - 1) * m_randFrequency;
	// TODO: Update the way time is handled.
	bool allowHit = (gEnv->pTimer->GetCurrTime() - m_lastSpawnTime) > fFrequency;
	if (m_autoSpawn && allowHit)
		ProcessHit(false);

#if WATER_RIPPLES_EDITING_ENABLED
	//	const bool debugDraw = m_enabled && gEnv->IsEditing() && g_pGame->DisplayEditorHelpersEnabled();
	const bool debugDraw = m_enabled && gEnv->IsEditing();
	if (debugDraw)
	{
...
void CWaterRipplesGenerator::RegisterEvents(IGameObjectExtension& goExt, IGameObject& gameObject)
{
...
void CWaterRipplesGenerator::ProcessHit(bool isMoving)
{
	if ((m_enabled))
	{
		...
		// No random offsets during movement spawning.
		if (!isMoving)
		{
			vWorldPos += Vec3(m_randomOffset.x * (cry_random(0.0f, 1.0f) * 2 - 1), m_randomOffset.y * (cry_random(0.0f, 1.0f) * 2 - 1), 0.0f);
		}

		float fScale = m_scale + (cry_random(0.0f, 1.0f) * 2 - 1) * m_randScale;
		float fStrength = m_strength + (cry_random(0.0f, 1.0f) * 2 - 1) * m_randStrength;
		// HACK: TODO: ILH - removed just so I can get a test working on this code for 5.2. Replace with new code
		// when github updates.
...
void CWaterRipplesGenerator::ActivateGeneration(const bool activate)
{
	...
	if (activate && (gEnv->IsEditor() || m_autoSpawn))
	{
...
#if WATER_RIPPLES_EDITING_ENABLED
bool CWaterRipplesGenerator::TestLocation(const Vec3& testPosition)
{
