	...
	m_pTargetOptionalDatabase(pTargetOptionalDatabase)
{
	...
	m_pTargetActor = CActor::GetActor(m_targetEntityId);

	// Determine a suitable scope context.
	if (m_pTargetActor)
		m_scopeContextId = g_actorMannequinParams.contextIDs.SlaveCharacter;
	else
		m_scopeContextId = g_actorMannequinParams.contextIDs.SlaveObject;
}
