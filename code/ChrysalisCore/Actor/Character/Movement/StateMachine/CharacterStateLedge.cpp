...
public:
	...
	CActionLedgeGrab(CCharacter &Character, const QuatT &ledgeLoc, SLedgeTransitionData::EOnLedgeTransition transition, bool endCrouched,
		...
		: CAnimationAction(EActorActionPriority::eAAP_ActionUrgent, g_actorMannequinParams.fragmentIDs.LedgeGrab),
	...
	virtual void OnInitialise()
	{
		...
		if (fragTagDef)
		{
			const SActorMannequinParams::Fragments::SledgeGrab& ledgeGrabFragment = g_actorMannequinParams.fragments.ledgeGrab;
...
bool CCharacterStateLedge::CanGrabOntoLedge(const CCharacter& Character)
{
	if (Character.IsClient())
	{
		/*		const float kAirTime [2] = {0.5f, 0.0f};
				const float inAirTime = Character.CCharacter::GetActorState ()->durationInAir;
