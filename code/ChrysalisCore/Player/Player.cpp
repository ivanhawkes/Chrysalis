...
void CPlayer::AttachToCharacter(EntityId characterId)
{
	...
	auto pCharacter = static_cast<CCharacter*>(CActor::GetActor(characterId));
	if (pCharacter)
	{
		pCharacter->OnPlayerAttach(*this);

		// Flip the camera between FP / TP if needed.
		if (pCharacter->IsThirdPerson() != m_pCameraManager->IsThirdPerson())
			m_pCameraManager->ToggleThirdPerson();
	}
}
...
IActor* CPlayer::GetAttachedActor() const
{
	...
	return CActor::GetActor(m_attachedCharacterId);
}
...
CCharacter* CPlayer::GetAttachedCharacter() const
{
	...
	return static_cast<CCharacter*> (CActor::GetActor(m_attachedCharacterId));
}
