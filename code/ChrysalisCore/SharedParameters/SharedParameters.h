...
\param	rootParams	Generally this will be the root of the XML document that contains the struct values. In some
					cases it might be preferable to pass in a sub-document.
\param	className 	Used for creating a unique hash index for the shared parameter. Pass in the general class of the
					struct e.g. item, weapon.
\param	entityName	Used for creating a unique hash index for the shared parameter. Pass in the name of the entity
					which will own this parameter. This allows each 'class' and 'entity' to have it's own unique
					shared parameter group.
\param	nodeName  	Name of the XML node which will we will read the parameters from. This is expected to be a sub-
					node of the root node passed in.
