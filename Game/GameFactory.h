...
public:
	...
	\param	name The name.
	\param	isAi true if this instance is ai.
	**/
	template<class T>
	static void RegisterFactory(const char *name, bool isAi)
	{
		gEnv->pGame->GetIGameFramework()->RegisterFactory(name, static_cast<T*> (nullptr), isAi, static_cast<T*> (nullptr));
	}
