...
#include "Boids/Avian/ChickenBoids.h"
#include "Boids/Amphibian/FrogBoids.h"
...
//////////////////////////////////////////////////////////////////////////
CScriptBindBoids::CScriptBindBoids()
{
	CScriptableBase::Init(GetISystem()->GetIScriptSystem(), GetISystem());
	SetGlobalName( "Boids" );

	// For Xennon.
#undef SCRIPT_REG_CLASSNAME
#define SCRIPT_REG_CLASSNAME &CScriptBindBoids::
...
//////////////////////////////////////////////////////////////////////////
CScriptBindBoids::~CScriptBindBoids(void)
{
}

//////////////////////////////////////////////////////////////////////////
IEntity* CScriptBindBoids::GetEntity( IScriptTable *pEntityTable )
{
	if (!pEntityTable)
		return 0;
...
//////////////////////////////////////////////////////////////////////////
CFlock* CScriptBindBoids::GetFlock( IScriptTable *pEntityTable )
{
	if(boids_enabled == 0)
		return 0;
...
//////////////////////////////////////////////////////////////////////////
int CScriptBindBoids::CreateFlock( IFunctionHandler *pH, SmartScriptTable entity,int nType,SmartScriptTable paramTable )
{
	if(boids_enabled == 0)
		return 0;
	...
	switch (nType) {
	case EFLOCK_BIRDS:
		pFlock = new CAvianFlock(pEntity);
		break;
...
//////////////////////////////////////////////////////////////////////////
int CScriptBindBoids::SetFlockParams(IFunctionHandler *pH,SmartScriptTable entity,SmartScriptTable paramTable)
{
...
//////////////////////////////////////////////////////////////////////////
int CScriptBindBoids::EnableFlock(IFunctionHandler *pH,SmartScriptTable entity,bool bEnable)
{
	CFlock *flock = GetFlock(entity);
	if (flock)
...
int CScriptBindBoids::SetFlockPercentEnabled(IFunctionHandler *pH,SmartScriptTable entity,int percent )
{
	CFlock *flock = GetFlock(entity);
...
int CScriptBindBoids::SetAttractionPoint(IFunctionHandler *pH,SmartScriptTable entity, Vec3 point )
{
	CAvianFlock *pFlock = (CAvianFlock *)GetFlock(entity);
	if (pFlock)
...
//////////////////////////////////////////////////////////////////////////
bool CScriptBindBoids::ReadParamsTable(IScriptTable *pTable, struct SBoidContext &bc,SBoidsCreateContext &ctx )
{
...
//////////////////////////////////////////////////////////////////////////
int CScriptBindBoids::OnBoidHit( IFunctionHandler *pH,SmartScriptTable flockEntity,SmartScriptTable boidEntity,SmartScriptTable hit )
{
...
//////////////////////////////////////////////////////////////////////////
int CScriptBindBoids::CanPickup(IFunctionHandler *pH, SmartScriptTable flockEntity, SmartScriptTable boidEntity)
{
	CFlock* flock = GetFlock(flockEntity);
	IEntity* pBoidEntity = GetEntity(boidEntity);
...
//////////////////////////////////////////////////////////////////////////
int CScriptBindBoids::GetUsableMessage(IFunctionHandler *pH, SmartScriptTable flockEntity)
{
	CFlock* flock = GetFlock(flockEntity);
	const auto pPlayer = CPlayer::GetLocalPlayer();
...
//		if (pPlayer && !pPlayer->IsInPickAndThrowMode() && flock != NULL)
		// TODO: fix the up so it checks the character for pick and throw mode. It's broken until then.
		if (pPlayer && flock != NULL)
		{
			SBoidContext bc;
			flock->GetBoidSettings(bc);

			tempString.Format("@ui_boid_pickup %s", bc.pickableMessage);
			finalString = LocalizeUtility::LocalizeString(tempString.c_str());
		}
...
//////////////////////////////////////////////////////////////////////////
int CScriptBindBoids::OnPickup(IFunctionHandler *pH, SmartScriptTable flockEntity, SmartScriptTable boidEntity, bool bPickup, float fThrowSpeed)
{
	CFlock* flock = GetFlock(flockEntity);
	IEntity* pBoidEntity = GetEntity(boidEntity);
...
/*
int CScriptBindBoids::Land(IFunctionHandler *pH, SmartScriptTable flockEntity)
{
...
int CScriptBindBoids::TakeOff(IFunctionHandler *pH, SmartScriptTable flockEntity)
{
	CFlock *pFlock = GetFlock(flockEntity);
	if(pFlock)
	{
