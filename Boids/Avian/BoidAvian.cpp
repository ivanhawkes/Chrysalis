...
float CBoidAvian::m_TakeOffAnimLength = 0.f;

CBoidAvian::CBoidAvian(SBoidContext &bc)
: CBoidObject( bc )
{
	m_actionTime = 0;
	m_lastThinkTime = 0;
...
CBoidAvian::~CBoidAvian()
{
	m_floorCollisionInfo.Reset();
}

void CBoidAvian::OnFlockMove( SBoidContext &bc )
{
	m_birdOriginPos = bc.flockPos;
...
//////////////////////////////////////////////////////////////////////////
void CBoidAvian::Physicalize( SBoidContext &bc )
{
...
//////////////////////////////////////////////////////////////////////////
void CBoidAvian::UpdatePhysics( float dt,SBoidContext &bc )
{
	if (m_pPhysics)
	{
		pe_status_pos ppos;
...
//////////////////////////////////////////////////////////////////////////
void CBoidAvian::Update( float dt,SBoidContext &bc )
{
...

	{
		...
		if(m_status == Avian::TAKEOFF)
		{
			...
			if(m_playingTakeOffAnim &&  timePassed >= m_TakeOffAnimLength)
			{
				...
				PlayAnimationId(Avian::ANIM_FLY,true);
			}
			...
			if(dist > 0.02f && m_pos.z > m_landingPoint.z)
			{
				...
				if(m_landDecelerating)
				{
					...
					if(!wasLandDeceleratingAlready)
						PlayAnimationId(Avian::ANIM_LANDING_DECELERATING, true);
				}
				else
		...
		{
			if(bc.walkSpeed > 0 && m_onGroundStatus == Avian::OGS_WALKING)
				ThinkWalk(dt,bc);
			CalcMovementBird( dt,bc,true );
		}
		...
		if ( m_status == Avian::LANDING ||(m_dying && m_status != Avian::ON_GROUND))
		{
		...
		else
		{
			if(!bc.noLanding && m_actionTime > m_maxActionTime 
				&& !static_cast<CAvianFlock*>(m_flock)->IsPlayerNearOrigin())
				Land();
		}
	}
}
...
void CBoidAvian::UpdateOnGroundAction(float dt, SBoidContext& bc)
{
	if(m_actionTime > m_maxActionTime)
	{
		...
		case Avian::OGS_IDLE:
			...
			if(bc.walkSpeed > 0)
			{
				m_onGroundStatus = Avian::OGS_WALKING;
				PlayAnimationId(Avian::ANIM_WALK,true);
				m_actionTime = 0;
		...
		case Avian::OGS_WALKING:
			// start slowing down until we come to a rest
			m_onGroundStatus = Avian::OGS_SLOWINGDOWN;
			m_elapsedSlowdownTime = 0.0f;
			break;

		default:
			CRY_ASSERT_MESSAGE(0, "CBoidAvian::UpdateOnGroundAction: omitted EOnGroundStatus");
		}
	}
}	
...
void CBoidAvian::ClampSpeed(SBoidContext& bc,float dt)
{
	if(m_status == Avian::ON_GROUND)
	{
		if(m_onGroundStatus == Avian::OGS_WALKING)
		{
			m_speed = m_walkSpeed;
		}
		else if(m_onGroundStatus == Avian::OGS_SLOWINGDOWN)
		{
		...
		else if(m_onGroundStatus == Avian::OGS_IDLE)
		{
			m_speed = 0.0f;
		}
	}
...
void CBoidAvian::CalcMovementBird(float dt, SBoidContext& bc, bool banking)
{
...
void CBoidAvian::SetStatus(Avian::EStatus status)
{
	...
	if(m_status != Avian::LANDING && status == Avian::LANDING)
	{
		m_landDecelerating = 0;
	}
	m_status = status;
	m_collisionInfo.SetCheckDistance(status == Avian::LANDING ? 5.f : 10.f);
}

//////////////////////////////////////////////////////////////////////////

void CBoidAvian::Land()
{
	if(m_status != Avian::ON_GROUND && m_status!= Avian::LANDING)
	{
		SLandPoint lpt = static_cast<CAvianFlock*>(m_flock)->GetLandingPoint(m_pos);
		m_landingPoint = lpt;
		if(!lpt.IsZero())
		{
			m_startLandSpeed = m_speed;
			SetStatus(Avian::LANDING);
		}
	}
}
...
void CBoidAvian::Landed(SBoidContext& bc)
{
	SetStatus(Avian::ON_GROUND);
	static_cast<CAvianFlock*>(m_flock)->NotifyBirdLanded();
	m_actionTime = 1000;
	m_accel.zero();
	m_heading.z = 0;
...
//////////////////////////////////////////////////////////////////////////
void CBoidAvian::TakeOff( SBoidContext &bc )
{
	// Take-off.
	if(m_status != Avian::LANDING && m_status != Avian::ON_GROUND)
		return;
	
	SetStatus(Avian::TAKEOFF);

	static_cast<CAvianFlock*>(m_flock)->LeaveLandingPoint(m_landingPoint);
...
//	m_maxActionTime = MIN_FLIGHT_TIME + (Boid::Frand()+1)/2*(MAX_FLIGHT_TIME-MIN_FLIGHT_TIME);
	...
	if (m_object)
	{
		const char* takeoffName = bc.GetAnimationName(Avian::ANIM_TAKEOFF);
...
void CBoidAvian::UpdatePitch(float dt,SBoidContext& bc)
{
	Vec3 orientationGoal;
	if(m_status == Avian::LANDING && m_landDecelerating)
	{
	...
	else if (m_status == Avian::ON_GROUND && m_heading.z!=0)
	{
		orientationGoal = m_heading;
		orientationGoal.z=0;
...
void CBoidAvian::Think( float dt,SBoidContext &bc )
{
	m_accel.zero();
	...
	// Avoid land.
	if (height < bc.MinHeight && m_status != Avian::LANDING)
	{
	...
	else if(m_status != Avian::TAKEOFF)
	{
	...
	if (m_pos.GetSquaredDistance2D(bc.attractionPoint) < 5.f)
	{
		...
		CAvianFlock *pFlock = (CAvianFlock *)m_flock;
...
//////////////////////////////////////////////////////////////////////////
void CBoidAvian::ThinkWalk( float dt,SBoidContext &bc )
{
...
//////////////////////////////////////////////////////////////////////////
void CBoidAvian::UpdateAnimationSpeed(SBoidContext &bc)
{
	if(!m_object)
		return;
	...
	case Avian::ON_GROUND:
		{
			if(bc.walkSpeed > 0 && m_onGroundStatus != Avian::OGS_IDLE)
...
//////////////////////////////////////////////////////////////////////////
void CBoidAvian::Kill( const Vec3 &hitPoint,const Vec3 &force )
{
	if (m_dead)
		return;
	...
	if (pEntity)
	{
		...
		gEnv->pEntitySystem->GetBreakableManager()->AttachSurfaceEffect(pEntity, 0, SURFACE_BREAKAGE_TYPE("destroy"), params);
	}
...
bool CBoidAvian::ShouldUpdateCollisionInfo(const CTimeValue& t)
{
	if(m_status == Avian::LANDING && m_landDecelerating  )
		return false;
...
/*
void CBoidAvian::OnGrab( )
{
	//CBoidObject::OnGrab();

	m_status = Avian::PICKED_UP;
	PlayAnimationId(Avian::ANIM_GRABBED,true);
}

/////////////////////////////////////////////////////////////

void CBoidAvian::OnThrow()
{
	Kill(ZERO,ZERO);
}*/
