...
// Ledge definition structures, types and flags. These flags are exported with exported levels. Do NOT introduce new
// flags in the middle of these or you'll require ALL levels to be re-exported to ensure the correct flags are present.
enum
{
	kLedgeFlag_none = 0,
...
public:
	...
	ILINE bool IsInEditorMode() const { return m_inEditorMode; }
	ILINE void Reset() { m_ledgeObjects.clear(); }

	void RegisterLedge(EntityId entityId, const SLedgeMarker* pMarkersArray, const uint32 markerCount, ELedgeFlagBitfield ledgeInFlags,
		ELedgeFlagBitfield ledgeOutFlags, float ledgeCornerMaxAngle, float ledgeCornerEndAdjustAmount);
	...
	LedgeId FindNearestLedge(const Vec3 &referencePosition, const Vec3 &testDirection, const float maxDistance, const float angleRange,
		const float extendedAngleRange) const;
...
public:
	...
	ILINE LedgeId FindNearestLedge(const Vec3 &vPos, const Vec3 &vDir, float fMaxDistance = 2.0f, float fAngleRange = DEG2RAD(35.0f),
		float fExtendedAngleRange = DEG2RAD(50.0f)) const
	{
...
Editor:
While in editor the data is stored in a slightly different format, less efficient, but easier for editing purposes.
...
public:
	...
	{
		return m_editorManager.IsInEditorMode() ? &m_editorManager : nullptr;
	}
