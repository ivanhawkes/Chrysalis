...
NOTE: If you use "SetStance" you can't also implement the CHARACTER_EVENT_STANCE_CHANGED within the same hierarchy!
...
const CCharacterStateMovement::TStateIndex CCharacterStateMovement::Root(CCharacter& Character, const SStateEvent& event)
{
	...
	switch (eventID)
	{
		...
		case CHARACTER_EVENT_BECOME_LOCAL_PLAYER:
			CreateWaterEffects();
			break;
...
const CCharacterStateMovement::TStateIndex CCharacterStateMovement::Ladder(CCharacter& Character, const SStateEvent& event)
{
	...
	{
		case STATE_EVENT_ENTER:
			m_flags.AddFlags(ECharacterStateFlags_OnLadder);
			break;

		case STATE_EVENT_EXIT:
			m_flags.ClearFlags(ECharacterStateFlags_OnLadder);
...
/*	CRY_ASSERT(!Character.IsAIControlled());
	...
	switch (eventID)
	{
		case STATE_EVENT_ENTER:
			...
			m_flags.AddFlags(ECharacterStateFlags_Spectator);
			break;

		case STATE_EVENT_EXIT:
			m_flags.ClearFlags(ECharacterStateFlags_Spectator);
	...
	switch (eventID)
	{
		...
		case CHARACTER_EVENT_PREPHYSICSUPDATE:
		{
			...
			m_stateGround.OnPrePhysicsUpdate(Character, prePhysicsEvent.m_movement, prePhysicsEvent.m_frameTime,
				m_flags.AreAnyFlagsActive(ECharacterStateFlags_CurrentItemIsHeavy), Character.IsPlayer());
		}
			break;

		case CHARACTER_EVENT_JUMP:
			return State_Jump;
...
const CCharacterStateMovement::TStateIndex CCharacterStateMovement::Jump(CCharacter& Character, const SStateEvent& event)
{
	...
	switch (eventID)
	{
		case STATE_EVENT_ENTER:
			...
			m_flags.AddFlags(ECharacterStateFlags_InAir | ECharacterStateFlags_Jump);
			m_flags.ClearFlags(ECharacterStateFlags_Sprinting);
			break;
		...
		case CHARACTER_EVENT_JUMP:
		{
			...
			m_stateJump.OnJump(Character, m_flags.AreAnyFlagsActive(ECharacterStateFlags_CurrentItemIsHeavy), fVerticalSpeedModifier);

			const SCharacterPrePhysicsData& data = static_cast<const SStateEventJump&> (event).GetPrePhysicsEventData();
			Character.StateMachineHandleEventMovement(SStateEventCharacterMovementPrePhysics(&data));
		}
			break;
		...
		case CHARACTER_EVENT_PREPHYSICSUPDATE:
		{
			...
			//	ledge.m_comingFromSprint = m_flags.AreAnyFlagsActive(ECharacterStateFlags_Sprinting);
...
/*	CRY_ASSERT(!Character.IsAIControlled());
	...
	switch (eventID)
	{
		case STATE_EVENT_ENTER:
			...
			m_flags.AddFlags(ECharacterStateFlags_Sliding);
			m_flags.ClearFlags(ECharacterStateFlags_Sprinting);
			break;
		...
		case STATE_EVENT_EXIT:
			m_flags.ClearFlags(
				ECharacterStateFlags_Sprinting |
				ECharacterStateFlags_ExitingSlide |
				ECharacterStateFlags_NetExitingSlide |
				ECharacterStateFlags_NetSlide |
				ECharacterStateFlags_Sliding);
		...
		case CHARACTER_EVENT_PREPHYSICSUPDATE:
		{
			...
			m_slideController.Update(Character, prePhysicsEvent.m_frameTime, prePhysicsEvent.m_movement, m_flags.AreAnyFlagsActive(ECharacterStateFlags_NetSlide), m_flags.AreAnyFlagsActive(ECharacterStateFlags_NetExitingSlide), Character.GetMoveRequest());

			m_slideController.GetCurrentState() == eSlideState_ExitingSlide ? m_flags.AddFlags(ECharacterStateFlags_ExitingSlide) : m_flags.ClearFlags(ECharacterStateFlags_ExitingSlide);
...
/*	CRY_ASSERT(!Character.IsAIControlled());
	...
	{
		case STATE_EVENT_ENTER:
			m_flags.AddFlags(ECharacterStateFlags_Ledge);
		...
		case STATE_EVENT_EXIT:
			m_flags.ClearFlags(ECharacterStateFlags_Ledge);
		...
		case CHARACTER_EVENT_LEDGE:
		{
			...
			if (comingFromSprint)
			{
				m_flags.AddFlags(ECharacterStateFlags_Sprinting); // movement will have stripped this flag as it exited.. we want to carry on sprinting at the end of this sprint vault if possible
			}
		...
		case CHARACTER_EVENT_LEDGE_ANIM_FINISHED:
...
void CCharacterStateMovement::StateSprintInput(CCharacter& Character, const SInputEventData& inputEvent)
{
	...
	inputEvent.m_activationMode == eAAM_OnPress ? m_flags.AddFlags(ECharacterStateFlags_SprintPressed)
		: m_flags.ClearFlags(ECharacterStateFlags_SprintPressed);
}


void CCharacterStateMovement::ProcessSprint(CCharacter& Character, const SCharacterPrePhysicsData& prePhysicsEvent)
{
